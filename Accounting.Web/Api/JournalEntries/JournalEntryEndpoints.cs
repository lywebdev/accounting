...
public static class JournalEntryEndpoints
{
    public static IEndpointRouteBuilder MapJournalEntryEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/", async (DateOnly? from, DateOnly? to, IJournalService service, CancellationToken ct) =>
        {
            var entries = await service.GetAsync(from, to, ct);
            return Results.Ok(entries.Select(e => e.ToDto()));
        });

        group.MapPost("/", async (CreateJournalEntryRequest request, IJournalService service, CancellationToken ct) =>
        {
        ...
        group.MapDelete("/{id:guid}", async (Guid id, IJournalService service, CancellationToken ct) =>
        {
